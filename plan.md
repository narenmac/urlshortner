...
| IaC | Terraform (local state for dev) |
...
- **Cost**: Azure Table Storage is very cheap; AKS cost depends on node size/count
- **Scalability**: HPA on url-service handles load spikes; Azure Table Storage scales automatically
- **Redis (future)**: Can add Azure Cache for Redis (~$40/mo) later for caching when budget allows

---

## Deployment Modifications & Learnings (Post-Implementation)

The following changes were made during actual deployment to Azure:

### Redis Removed (Cost Optimization)
- Azure Cache for Redis has no free tier (~$40/month minimum)
- **Decision**: Removed Redis entirely; app goes directly to Azure Table Storage
- **Impact**: Slightly higher latency on reads (~20-50ms vs ~1-3ms with cache)
- **Future**: Can re-add Redis as a pod on AKS (free) or Azure Cache when budget allows

### Terraform Backend
- Switched from remote Azure Blob backend to **local state** for simplicity
- Remote backend requires a pre-existing Storage Account for state — unnecessary overhead for personal testing
- Can migrate to remote backend later for team collaboration

### Docker Build Fix (Multi-Module Maven)
- **Problem**: Docker context for each service couldn't access parent `pom.xml`
- **Solution**: Set Docker context to `./backend` (parent directory) with explicit `dockerfile` path
- Both Dockerfiles copy ALL module POMs + sources, then build only their target module with `mvn -pl <module> -am`

### Azure Table Storage API Fix
- `TableClient.createTableIfNotExists()` doesn't exist in azure-data-tables SDK
- **Fix**: Use `TableServiceClient.createTableIfNotExists(tableName)` then get `TableClient` from it

### Kubernetes Deployment Fixes

| Issue | Root Cause | Fix |
|-------|-----------|-----|
| Namespaces not found | Secrets/ConfigMaps created before namespaces | Added "Create Namespaces" step first in CD |
| Pods stuck in Pending | `nodeSelector: tier: backend` but AKS had no labeled nodes | Removed nodeSelectors for manual AKS |
| HPA invalid | `minreplicas`/`maxreplicas` (wrong case) | Fixed to `minReplicas`/`maxReplicas` |
| Ingress invalid service name | `gateway-service.backend` (dots not allowed) | Created ExternalName service as cross-namespace proxy |
| Replicas resource pressure | 2 replicas per service on small test cluster | Reduced to 1 for testing |

### Networking: Cross-Namespace Routing
- **Problem**: Ingress in `frontend` namespace can't directly reference services in `backend` namespace
- **Solution**: ExternalName service (`gateway-proxy`) in frontend namespace that DNS-resolves to `gateway-service.backend.svc.cluster.local`
- **Flow**: Ingress → `gateway-proxy` (ExternalName) → gateway-service in backend namespace

### Short URL Path
- **Problem**: Short URLs like `http://IP/abc123` routed to React frontend (which doesn't handle them)
- **Solution**: Changed short URL format to `http://IP/api/r/abc123` (routed through Ingress → Gateway)
- The `/api/r/{code}` endpoint returns a 302 redirect to the original URL

### API Key Filter Fix
- **Problem**: Filter hardcoded `https://ourdomain` as allowed origin; blocked requests from `http://IP`
- **Solution**: Compare Origin header against actual Host header dynamically; allow same-origin requests

### APP_BASE_URL Auto-Detection
- **Problem**: ConfigMap had hardcoded `https://ourdomain` as base URL for short links
- **Solution**: CD pipeline auto-detects Ingress LoadBalancer IP and sets `APP_BASE_URL` dynamically

### GitHub Actions Key Learnings
- **Runners**: Uses `runs-on: ubuntu-latest` (GitHub-hosted, free for public repos, no setup needed)
- **Secrets**: All sensitive values stored in GitHub Settings → Secrets → Actions (never in code)
- **Service Principal**: Created via Azure Portal (Entra ID → App registrations → new secret → assign Contributor role to resource group)
- **Docker Hub**: Uses Access Token (not password) for authentication
...
| `AKS_RESOURCE_GROUP` | `url-shortener-rg` |
